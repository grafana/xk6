...
- the `xk6-output-`, `xk6-subcommand-` and `xk6-` prefixes are removed
...
The `grafana/xk6-example`, `grafana/xk6-output-example` and `grafana/xk6-subcommand-example` GitHub repositories are used as sources for generation. Certain files are automatically excluded from the template: `.git` directory, `CODEOWNERS`, and `renovate.json`. These files are specific to the template repository itself and not needed for new extensions.
...
```
  -t, --type string          The type of template to use (javascript, output, subcommand)
